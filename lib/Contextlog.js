...
class Contextlog {
  constructor(options) {
    ...
    this.MASK_HEADERS = ['authorization'];
  ...
  getRequestResponseInfo(req, res) {
    ...
    if (this.options.reqBody) {
      if (typeof this.options.reqBody === 'function' && req.body) {
        meta.req.body = this.options.reqBody(req.body);
      } else {
    ...
    if (this.options.reqBody) {
      if (typeof this.options.resBody === 'function' && res.body) {
        meta.res.body = this.options.resBody(res.body);
      } else {
        meta.res.body = res.body;
      }
    }

    if (this.options.parseUserAgent) {
      const userAgent = useragent.parse(req.headers['user-agent']);
      meta.req.userAgent = userAgent.toJSON();
    }
